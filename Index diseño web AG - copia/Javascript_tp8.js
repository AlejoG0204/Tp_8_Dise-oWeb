...
// ===================================
// SISTEMA DE CONTADOR DE PRODUCTOS
// ===================================
...
class ContadorProductos {
    ...
    agregarEventosProductos() {
        ...
        productos.forEach((producto, index) => {
            ...
            const idProducto = `producto_${index}`;
            
            // Crear elementos de controles
            const contador = document.createElement('div');
            contador.className = 'product-counter';
            contador.dataset.id = idProducto;
            
            const controles = document.createElement('div');
            controles.className = 'quantity-controls';
            controles.dataset.id = idProducto;
            
            const btnAnadir = document.createElement('button');
            btnAnadir.className = 'add-to-cart-btn';
            btnAnadir.textContent = 'Añadir al carrito';
            btnAnadir.dataset.id = idProducto;
            
            // Insertar elementos
            producto.appendChild(contador);
            producto.appendChild(btnAnadir);
            producto.appendChild(controles);
            
            // Evento del botón "Añadir"
            btnAnadir.addEventListener('click', () => {
                this.agregarProducto(idProducto, nombreProducto);
            });
        });
    ...
    agregarProducto(idProducto, nombreProducto) {
        ...
        // Notificación
        this.mostrarNotificacion(`${nombreProducto} agregado al carrito`);
    }
    ...
    decrementar(idProducto) {
        if (this.carrito[idProducto] && this.carrito[idProducto].cantidad > 0) {
            this.carrito[idProducto].cantidad--;
    ...
    actualizarContadores() {
        Object.keys(this.carrito).forEach(idProducto => {
            const contador = document.querySelector(`[data-id="${idProducto}"].product-counter`);
            const cantidad = this.carrito[idProducto].cantidad;
    ...
    actualizarControles() {
        ...
        productos.forEach((producto) => {
            const btnAnadir = producto.querySelector('.add-to-cart-btn');
            const controles = producto.querySelector('.quantity-controls');
            const contador = producto.querySelector('.product-counter');
            const idProducto = contador.dataset.id;
            
            if (this.carrito[idProducto] && this.carrito[idProducto].cantidad > 0) {
                btnAnadir.style.display = 'none';
                controles.classList.add('active');
                
                // Crear botones si no existen
                if (!controles.innerHTML) {
                    controles.innerHTML = `
                        <button class="quantity-btn minus-btn" data-id="${idProducto}">−</button>
                        <span class="quantity-display">${this.carrito[idProducto].cantidad}</span>
                        <button class="quantity-btn plus-btn" data-id="${idProducto}">+</button>
                    `;
                    
                    // Eventos de los botones
                    controles.querySelector('.minus-btn').addEventListener('click', () => {
                        this.decrementar(idProducto);
                    });
                    
                    controles.querySelector('.plus-btn').addEventListener('click', () => {
                        this.incrementar(idProducto);
                    });
                } else {
                    // Actualizar cantidad mostrada
                    controles.querySelector('.quantity-display').textContent = this.carrito[idProducto].cantidad;
                }
            } else {
                btnAnadir.style.display = 'block';
                controles.classList.remove('active');
            }
        });
    }
    ...
    actualizarCartTotal() {
        const total = Object.values(this.carrito).reduce((sum, item) => sum + item.cantidad, 0);
...
document.addEventListener('DOMContentLoaded', () => {
    const contadorProductos = new ContadorProductos();
});